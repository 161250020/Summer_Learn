...
public class 丑数 {
    ...
    */
    ...
    public int GetUglyNumber_Solution(int index) {
        ...
        arrayList.add(newNum);//先添加1进去，作为初始化
        while (arrayList.size()<index){
            //选出最小的那个数
            newNum=Math.min(arrayList.get(p2)*2,
                    Math.min(arrayList.get(p3)*3,arrayList.get(p5)*5));
            if (arrayList.get(p2)*2==newNum)p2++;
            if (arrayList.get(p3)*3==newNum)p3++;
            if (arrayList.get(p5)*5==newNum)p5++;
